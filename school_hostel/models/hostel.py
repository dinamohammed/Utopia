...
class HostelStudent(models.Model):
    ...
    def invoice_view(self):
        ...
        for rec in self:
            ...
            action = rec.env.ref('account.action_move_tree1').read()[0]
            if len(invoices) > 1:
                action['domain'] = [('id', 'in', invoices.ids)]
            elif len(invoices) == 1:
                action['views'] = [(rec.env.ref('account.move_form').id,
                                    'form')]
    ...
    def pay_fees(self):
        ...
        for rec in self:
            ...
            account_inv_id.write({'line_ids': [(0, 0, line_vals)]})
...
class AccountMove(models.Model):
    ...
    account_id = fields.Many2one('account.account',string="Account Receivable")
