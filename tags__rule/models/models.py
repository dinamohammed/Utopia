...
class TagsRule(models.Model):
    ...
    def create(self, vals):
        if vals.get('employee_id'):
            ...
            if emp_id.work_location_id:
                vals['work_location_id'] = emp_id.work_location_id.id
        if vals.get('contract_id'):
            contract_id = self.env['hr.contract'].browse(vals.get('contract_id'))
            if contract_id.contract_type_id:
                vals['contract_type_id'] = contract_id.contract_type_id.id
        return super(TagsRule, self).create(vals)
    ...
    def write(self, vals):
        ...
        if vals.get('contract_id'):
            ...
            if contract_id.contract_type_id:
                vals['contract_type_id'] = contract_id.contract_type_id.id
        return super(TagsRule, self).write(vals)
